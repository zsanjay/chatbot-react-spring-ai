...
public class ChatBotController {
    ...
    public ResponseEntity<AvatarUploadResponse> upload(@PathVariable("id") Long userId, @RequestBody AvatarImage avatarImage) {
        String image = null;
        try {
            image = storageService.upload(userId, avatarImage.image());
        } catch (Exception ex) {
            throw new ImageUploadException("Something went wrong while uploading avatar," + ex.getMessage());
        }
        return new ResponseEntity<>(new AvatarUploadResponse(true , image), HttpStatus.OK);
    }
